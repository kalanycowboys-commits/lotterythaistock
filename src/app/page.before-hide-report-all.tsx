...
const soldStock = sales
  .filter((sale) => sale.number === number)
...
if (availableStock < qty) {
  ...
  const updatedStock = stockItems
    .map((item) => {
      if (item.number !== number || remaining <= 0) {
        return item;
      }
...
const stockSummary = Object.values(
  stockItems.reduce(
    (acc, item) => {
      ...
      acc[item.number].received += item.quantity;
  ...
  const sold = sales
    .filter((sale) => sale.number === item.number)
    .reduce((sum, sale) => sum + sale.quantity, 0);

  return {
    ...item,
    sold,
    remaining: item.received - sold,
  };
});
  return (
    <main
...
>
  ...
  ขั้นตอนนี้เป็นช่องทดสอบรับข้อมูล
        ...
        <section
          style={{
            display: activeMenu === "stock" ? "grid" : "none",
            gridTemplateColumns:
...
<div style={{ marginBottom: "15px" }}>
  ...
  >
    กรอกเลขสลากเต็ม 6 หลัก เช่น 730640
  </small>
</div>
